...
Add static key-value pairs to the JWT payload. These are injected into every token generated by the plugin.

The following payload keys are reserved and cannot be overwritten: `iat`, `exp`, `email`, `id`, `site`, `username`, `iss`.
